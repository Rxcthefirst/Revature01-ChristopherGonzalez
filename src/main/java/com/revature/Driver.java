...
public class Driver {
	...
	Driver() {
		 ...
		 * tech called servlets.
		 */

		// Starts our server on port 8000 instead of default 8080;
		this.app = Javalin.create().start(8000);

		/*
		 * app.before(ctx -> { System.out.
		 * println("This happens before the http requests make it to their first intended resource."
		 * ); });
		 */
		 ...
		 * app.get("/user/{id}", ctx -> {
		 * 
		 * Set<User> users = new HashSet<>(); User user1 = new User(1, "Sean",
		 * "Mendelson", "Bossman007","Password", true); User user2 = new User(2,
		 * "Zeke","","", "", false); User user3 = new User(3, "Jen", "","","", false);
		 * 
		 * users.add(user1); users.add(user2); users.add(user3);
		 * 
		 * User selectedUser = null;
		 * 
		 * for (User u : users) { if (u.getId() ==
		 * Integer.parseInt(ctx.pathParam("id"))) selectedUser = u; }
		 * 
		 ...
		 * 
		 * System.out.println(receivedUser); ctx.status(HttpStatus.CREATED_201); });
		 * 
		 * app.after(ctx -> {
